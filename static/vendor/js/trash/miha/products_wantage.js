$(document).ready(function () {
    ...
    $("#order-btn").click(function (e) {
        e.preventDefault();
        var orderCreateUrl = $(this).data('url');


        var itemsInCart = []; // Массив товаров в корзине и их количества

        // Добавляем CSRF токен к данным запроса
        var csrfToken = $('[name=csrfmiddlewaretoken]').val();

        $.ajax({
            url: '/orders/check_inventory/',
            type: 'POST',
            headers: {
                'X-CSRFToken': csrfToken
            },
            data: {items: JSON.stringify(itemsInCart)},
            success: function (response) {
                ...
                if (unavailableProducts.length > 0) {
                    ...
                    window.location.href = orderCreateUrl;
                }
            },
