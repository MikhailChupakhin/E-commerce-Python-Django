...
class Product(models.Model):
    ...
    def get_absolute_url(self):
        return reverse('products:product_detail', kwargs={'category_slug': self.category.slug, 'subcategory_slug': self.sub_category.slug, 'product_slug': self.slug})
    ...
    def save(self, *args, **kwargs):
        ...
        # При сохранении объекта, вычисляем и устанавливаем итоговую стоимость
...
def check_image(sender, instance, **kwargs):
    if instance.image and os.path.basename(instance.image.path) != 'product_image_plug.webp':
        ...
        if aspect_ratio != 1 or not size_condition:
            # Записать путь к невалидному изображению
            ...
            print('Image removed due to conditions not met.')
